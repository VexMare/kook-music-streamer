...
try:
    ...
    import kookvoice_voice_fixed as kookvoice
    ...
    async def clear_list(msg: Message):
        try:
            ...
            player = kookvoice.Player(msg.ctx.guild.id)
            from kookvoice_voice_fixed import play_list
    ...
    async def help_cmd(msg: Message):
        from khl.card import Module, Card, CardMessage
        c = Card()
        c.append(Module.Header('恶小梦Bot网易接口 - 语音修复版本'))
        ...
        c.append(Module.Context('🔧 语音修复版本：解决语音连接问题'))
    ...
    print("建议在 Main 目录中运行: python test_files/run_bot_voice_fixed.py")
except Exception as e:
    print(f"❌ 启动失败: {e}")
    import traceback
    traceback.print_exc()
