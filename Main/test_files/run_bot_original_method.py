...
# -*- coding: utf-8 -*-
"""
KOOK音乐机器人启动脚本 - 使用原版播放方法
基于原版kookvoice_bot.py的播放逻辑
"""
...
try:
    ...
    async def join_vc(msg: Message):
        ...
        try:
            ...
            c.append(Module.Context('🔧 使用原版播放方法'))
            await msg.ctx.channel.send(CardMessage(c))
    ...
    async def help_cmd(msg: Message):
        from khl.card import Module, Card, CardMessage
        c = Card()
        c.append(Module.Header('恶小梦Bot网易接口 - 原版播放方法'))
        ...
        c.append(Module.Context('🔧 原版播放方法：使用原始kookvoice模块'))
    ...
    print("建议在 Main 目录中运行: python test_files/run_bot_original_method.py")
except Exception as e:
    print(f"❌ 启动失败: {e}")
    import traceback
    traceback.print_exc()
