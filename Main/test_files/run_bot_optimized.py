...
try:
    ...
    async def help_cmd(msg: Message):
        from khl.card import Module, Card, CardMessage
        c = Card()
        c.append(Module.Header('恶小梦Bot网易接口 - 优化版本'))
        ...
        c.append(Module.Context('🔧 优化版本：歌单实时获取URL，播放更流畅'))
    ...
    print("建议在 Main 目录中运行: python test_files/run_bot_optimized.py")
except Exception as e:
    print(f"❌ 启动失败: {e}")
    import traceback
    traceback.print_exc()
